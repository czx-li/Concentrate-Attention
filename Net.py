...
class BertPromptTune(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, labels, len_text):
        ...
        batch_attention = torch.zeros(len(attention[23]))
        for i in range(len(attention[23])):
            ...
            for layer, weight in zip([21, 22, 23], [1, 1, 1]):
                ...
                for j in range(16):
                    text_attention = sum(
                        attention[layer][i][j][mask_ids[1][i]][mask_ids[1][i] - 3 - len_text[i]: mask_ids[1][i] - 3])
                    Text_attention = Text_attention + text_attention
                Text_attention = Text_attention / 16
                layer_attention = layer_attention + Text_attention
            batch_attention[i] = layer_attention / 3
        ...
        return cls_logits, loss, batch_attention